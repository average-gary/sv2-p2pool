...
- ✅ **Observability**: Prometheus collector set (`IntCounter`s for material events, `IntCounter`Vec for `push_solution_dropped{reason}`, `IntGauge`s for cache sizes + sweeper-liveness timestamp) exposed via a built-in `/metrics` HTTP endpoint (`--metrics-addr`). Sibling `/healthz` for orchestrator probes. `--log-file` honoured via `init_logging`; submit_block log lines carry `request_id` + `template_id` for correlation with upstream JDP/TDP traffic.
...
- ✅ **IPC client crate**: connects to the upstream p2poolv2 IPC server, surfaces `validate_template` / `submit_solution` / `subscribe_chain_tip`. Server-side `submit_solution` is real (deserializes `rawBlock`, verifies `shareHash == block_hash()`). `subscribe_chain_tip` fans out tip changes from a `tokio::sync::watch::Receiver<BlockHash>` injected via `spawn_ipc_server_with_tip_source`; daemons without a wired tip source preserve the original stub behaviour. `validate_template` is still a placeholder stub awaiting a `ChainStoreHandle` in the daemon.
